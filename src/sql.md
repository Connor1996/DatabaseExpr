...
USE [TD-LTE]
GO
/****** Object:  Trigger [dbo].[tbCell_trigger]    Script Date: 2017/7/13 星期四 下午 1:56:00 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
alter trigger [dbo].[tbCell_trigger] on [dbo].[tbCell] instead of insert
as
	declare @sid nvarchar(255);
	select @sid=SECTOR_ID from inserted;
	delete from tbCell where SECTOR_ID = @sid;
...
```sql
alter trigger tbKPI_trigger on tbKPI instead of insert
as
	declare @nbr nvarchar(255);
	select @nbr=小区 from inserted;
	delete from tbKPI where 小区=@nbr;
...
```sql
alter trigger tbPRB_trigger on tbPRB instead of insert
as
	declare @nbr nvarchar(255);
	select @nbr=小区 from inserted;
	delete from tbKPI where 小区=@nbr;
...
```sql
alter trigger tbMROData_trigger on tbMROData for insert
as
	declare @ts nvarchar(30), @ss nvarchar(50), @is nvarchar(50);
	select @ts=[TimeStamp], @ss= ServingSector, @is=InterferingSector from inserted;
	delete from tbMROData where ServingSector=@ss and InterferingSector=@is and [TimeStamp] = @ts;
